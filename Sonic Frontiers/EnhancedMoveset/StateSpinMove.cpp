HOOK(bool, __fastcall, StateSpinMoveUpdate, m_SigStateSpinMoveUpdate(), int64_t in_pThis, int64_t in_pSonicContext)
{
	...
	float initialSpeed = (BlackboardHelper::IsSideView()
		? Configuration::SpinMoveVelocityThresholdSV
		: Configuration::SpinMoveVelocityThreshold);
	...
	ESonicState groundedState = ((horzVelocityMag >= (initialSpeed == 0 ? 15.0f : initialSpeed))
	...
	Player::SetAnimationParameter("SPEED", min(max(velocityMag / initialSpeed, 0.0f), 1.0f));

	if (!StateSpinBoost::Flags.test(EStateSpinBoostFlags_IsSpinMove))
	{
		// Exit into StateSpinBoost when exceeding terminal velocity.
		if (velocityMag >= maxSpeed + 5.0f)
		{
			if (Player::IsStateAvailableInView(Configuration::SpinMoveTerminalVelocityType))
			{
	...
	if (!isGrounded || horzVelocityMag >= initialSpeed)
	{
		Player::SetAnimationEffect(in_pSonicContext, true, "SPIN");
		Player::SetDamageSphere(in_pSonicContext, true);
	}
	else
	{
		Player::SetAnimationEffect(in_pSonicContext, false);
		Player::SetDamageSphere(in_pSonicContext, false);
	}
	...
	{
		// Exit into StateBumpJump if we become ungrounded.
		if (StateSpinMove::IsUngroundedExit())
		{
		...
		if (IsButtonPressed(EPlayerActionType_PlayerStomping) && Configuration::IsSpinMoveStompCancel)
		{
		...
		if (IsButtonPressed(EPlayerActionType_PlayerBoost) && Configuration::IsSpinMoveAirBoostCancel)
		{
...
HOOK(double, __fastcall, StateSpinMoveEnd, m_SigStateSpinMoveEnd(), int64_t in_pThis, int64_t in_pSonicContext, int a3)
{
	Player::SetDamageSphere(in_pSonicContext, false);
	Player::SetAnimationEffect(in_pSonicContext, false);

	return originalStateSpinMoveEnd(in_pThis, in_pSonicContext, a3);
}
