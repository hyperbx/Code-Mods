...
void IncreaseCharge()
{
	StateSpinBoostCharge::ChargeMultiplier += Configuration::SpinChargeMultiplier;

	// Reset Light Speed Dash charge timer.
...
HOOK(int64_t, __fastcall, StateSpinBoostChargeStart, m_SigStateSpinBoostChargeStart(), int64_t in_pThis, int64_t in_pSonicContext)
{
	...
	{
		// Only start camera when we're not charging Focus Dash.
...
HOOK(bool, __fastcall, StateSpinBoostChargeUpdate, m_SigStateSpinBoostChargeUpdate(), int64_t in_pThis, int64_t in_pSonicContext)
{
	...
	StateSpinBoostCharge::ChargeTime += Player::DeltaTime;

	// Gradually increase charge multiplier when holding charge input.
	if (IsButtonDown(EPlayerActionType_PlayerSonicboom) && Configuration::IsSpinChargeInputCharge)
	{
		StateSpinBoostCharge::ChargeMultiplier += std::lerp
		(
			0.0f,
			StateSpinBoostCharge::ChargeMultiplier + Configuration::SpinChargeHoldMultiplier,
			Player::DeltaTime
		);
	}
	...
	switch (Configuration::SpinChargeControlType)
	{
		...
		case ESpinChargeControlType_LostWorld:
		{
			...
			// Disallow SLW scheme for Focus Dash.
			if (!StateBulletDash::Flags.test(EStateBulletDashFlags_IsCharging))
			{
	...
	if (Configuration::IsSpinChargeBoostRelease && !StateBulletDash::Flags.test(EStateBulletDashFlags_IsCharging))
	{
...
{
	StateSpinBoostCharge::ChargeTime = 0.0f;
...
HOOK(double, __fastcall, _StateSpinBoostCharge_StateSpinBoostEnd, m_SigStateSpinBoostEnd(), int64_t in_pThis, int64_t in_pSonicContext, int a3)
{
	StateSpinBoostCharge::ReleaseVelocity = 0.0f;

	return original_StateSpinBoostCharge_StateSpinBoostEnd(in_pThis, in_pSonicContext, a3);
}
...
HOOK(bool, __fastcall, _StateSpinBoostCharge_GOCPlayerHsmGroundStateUpdate, m_SigGOCPlayerHsmGroundStateUpdate(), int64_t* in_pThis, ESonicState in_stateId)
{
	if (in_stateId == ESonicState_StateSpinBoost)
	{
		...
		// Reset charge multiplier if we entered StateSpinBoost without StateSpinBoostCharge.
		if (Player::LastState != ESonicState_StateSpinBoostCharge)
			StateSpinBoostCharge::ChargeMultiplier = 0.0f;
	}
...
HOOK(uint8_t, __fastcall, _StateSpinBoostCharge_PlayerKinematicsGroundUpdate, m_SigPlayerKinematicsGroundUpdate(), int64_t in_pPlayerPostureContext, app::player::GOCPlayerKinematicParams* in_pGOCPlayerKinematicParams, uint32_t& in_pFlags)
{
	auto forward = in_pGOCPlayerKinematicParams->Rotation.GetForward();

	if (StateSpinBoostCharge::Flags.test(EStateSpinBoostChargeFlags_IsReleased))
	{
		...
		StateSpinBoostCharge::ReleaseVelocity = std::clamp
		(
			initialVelocity * StateSpinBoostCharge::ChargeMultiplier,
			0.0f,
			Configuration::SpinChargeTerminalReleaseVelocity
		);

		StateSpinBoostCharge::Flags.reset(EStateSpinBoostChargeFlags_IsReleased);
	}
	else
	{
		// Store current forward direction so we can point the release direction correctly.
		StateSpinBoostCharge::ReleaseForward = forward;
	}

	// Gradually decay the release velocity on top of our current velocity.
	if (StateSpinBoostCharge::ReleaseVelocity > 0.0f)
	{
		StateSpinBoostCharge::ChargeMultiplier = 0.0f;
		StateSpinBoostCharge::ReleaseVelocity -= Configuration::SpinChargeVelocityDecayRate * Player::DeltaTime;

		in_pGOCPlayerKinematicParams->Velocity += forward * StateSpinBoostCharge::ReleaseVelocity;
	}

	return original_StateSpinBoostCharge_PlayerKinematicsGroundUpdate(in_pPlayerPostureContext, in_pGOCPlayerKinematicParams, in_pFlags);
}
...
HOOK(bool, __fastcall, _StateSpinBoostCharge_StateStompingDownUpdate, m_SigStateStompingDownUpdate(), int64_t in_pThis, int64_t in_pSonicContext)
{
	...
	if (IsButtonDown(EPlayerActionType_PlayerSonicboom) && !StateLightSpeedAttack::Flags.test(EStateLightSpeedAttackFlags_IsActive))
...
void StateSpinBoostCharge::Install()
{
	...
	INSTALL_HOOK(_StateSpinBoostCharge_StateSpinBoostEnd);
	INSTALL_HOOK(_StateSpinBoostCharge_GOCPlayerHsmGroundStateUpdate);
	INSTALL_HOOK(_StateSpinBoostCharge_PlayerKinematicsGroundUpdate);
